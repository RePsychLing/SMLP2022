...
The character in the upper right corner of this box is a toggle to collapse or expand the contents.
Try clicking on it.
:::
...
Because most students in the course have experience with [R](https://r-project.org) and the [RStudio](https://www.rstudio.com/products/rstudio/) integrated development environment (IDE) we will assume you have both of those installed.
...
The [quarto](https://quarto.org) sources for this website are available in a github [repository](https://github.com/RePsychLing/SMLP2022).
Clone this repository in a directory of your choice using the shell command
...
Mac and Linux users should visit [install.julialang.org](https://install.julialang.org/) in a browser. 
...
We mention this because we expect to use Julia v1.8.0 in the summer school but, at the time of writing this intro (late July, 2022), it is still at release-candidate status.

Once you have installed juliaup check the status.
...
If the release version is still in the 1.7 series then add the `rc` channel and make it the default.
...
### "Package mode" in the REPL as an alternative to calling `Pkg.` functions
...
The company [RStudio](https://rstudio.com) is being rebranded as [Posit](https://posit.co) to reflect its extended range of activities beyond the R language and the RStudio IDE.
:::

In a quarto markdown document (`.qmd` file extension) code cells can be R code or Python code or Julia code.
Python code cells and Julia code cells are executed using [Jupyter](https://jupyter.org) which means that the Jupyter "kernel" for Julia must be installed for evaluation of Julia code chunks.

::: {.callout-note collapse="true"}
### Jupyter

[Jupyter](https://jupyter.org) is an interactive notebook system for the browser that can execute code chunks in several different languages.
The name "Jupyter" is from fragments of the language names: `JU`lia, `PYT`hon, and `R`.
The process of rendering a Quarto document with Python or Julia code chunks begins by converting the document to a Jupyter notebook.
:::

The Jupyter kernel for Julia is part of [IJulia.jl](https://github.com/JuliaLang/IJulia.jl), a Julia package.

::: {.callout-note collapse="true"}
### Julia package repositories' names end in .jl
...
As the `IJulia` package may be used in many different projects, we add it to the global package list for the Julia release.

```{julia}
Pkg.activate()    # activate the system-wide configuration for the release
```
...
In my case the output is like (I am using the development quarto which has the artificial version 99.9.9)
...
The important thing to check is that Jupyter has been located as part of the Python 3 installation and that it has the expected Julia kernel(s).
If not, you need to add `python`, `python3` and `jupyter` from the `~/.julia/conda/3/bin/` directory to your search path.

## VS Code

The most popular editor for Julia code and for Quarto documents with Julia code chunks is [VS Code](https://code.visualstudio.com).
We recommend installing VS Code from the package for your operating system at the [download site](https://code.visualstudio.com/download).
