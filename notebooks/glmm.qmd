...
function logistic(η)
	expmeta = exp(-η)
	return inv(one(expmeta) + expmeta)
end
```
...
```{julia}
gm2 = let
  form = @formula(use ~ 1 + age*children + abs2(age) + children + urban + (1|dist))
  fit(MixedModel, form, contra, dist; nAGQ, contrasts, progress)
end
```
...
gm3 = let
  form = @formula(
    use ~ 1 + age * children + abs2(age) + children + urban + (1|urban&dist)
  )
  fit(MixedModel, form, contra, dist; nAGQ, contrasts, progress)
end
```
