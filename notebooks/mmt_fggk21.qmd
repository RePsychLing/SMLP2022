...
let
  ...
  draw(
    data(df2) * means1 + data(df) * lines1;
  )
end
```
...
let
  ...
  draw(
    data(dat2) * means2 + data(dat) * lines2;
  )
end
```
...
### 2.5 Fitting an overparameterized LMM
...
```{julia}
#| code-fold: true
gof_summary3 = DataFrame(
  dof=dof.([m1, m2, m2_nested]),
