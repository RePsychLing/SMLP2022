...
The `sleepstudy` data are from a study [@Balkin2000], [@Belenky2003] of the effects of sleep deprivation on response time.
...
@fig-xyplotsleep displays the data in a multi-panel plot created with the `lattice` package in `R`, using the [RCall](https://juliahub.com/ui/Packages/RCall/8GFyb/0.13.13) package for Julia.
...
m1 = let
  ...
  fit(MixedModel, form, sleepstudy; contrasts, progress)
end
```
...
This model has a slightly lower log-likelihood than does @lst-m1 and one fewer parameter than `m1`.
...
Interestingly, the model with fewer parameters, @lst-m2, has a greater sum of the leverage values than the model with more parameters, `m1`.
