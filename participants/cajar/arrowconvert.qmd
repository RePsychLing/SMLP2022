...
+ `ID`: Participant ID (stored as integer - should be a factor)
...
## Converting ID and Item columns to factors

Convert the integer `ID` values, which are between 1 and 156, to character strings of the form `S001` to `S156`, then convert these to a categorical array (similar to a factor in R).
...
```{julia}
Arrow.write("./data/IAT_data.arrow", iat; compress=:lz4)
```
...
>>> fea.read_table('./data/IAT_data.arrow')
pyarrow.Table
ID: dictionary<values=string, indices=int8, ordered=0> not null
...
> library(tibble)
> iat <- read_feather("./data/IAT_data.arrow")
> glimpse(iat)
Rows: 12,160
Columns: 7
$ ID         <fct> S001, S001, S001, S001, S001, S001, S001, S001, S001, S001,…
...
$ TaskSwitch <fct> Yes, No, Yes, No, Yes, No, No, Yes, No, Yes, Yes, No, No, N…
