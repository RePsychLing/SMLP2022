...
dat <- 
  ...
  select(Rvwr = reviewer_code, time=time_to_report_h, 
         Cond = treatment, 14, 17, 13, 19) |> 
  mutate(Rvwr = factor(Rvwr),
         time = time / 24.,  # convert from hours to days
         rating = remove_val_labels(recommendation),
         new = remove_val_labels(new_information),
         Rating = to_factor(recommendation),
         Cond = fct_relevel(Cond, "HH", "AH", "AA", "AL", "LL")) |> 
  select(Rvwr, Cond, time, Rating, rating, worthy=worthy_subject, new, 
...
ggplot(tbl1, aes(x = Rating, y = perc)) +
  geom_bar(
...
ggplot(tbl2, aes(x = Rating, y = perc)) +
  geom_bar(
    aes(color = Cond, fill = Cond),
   stat = "identity", position = position_dodge(0.8), width = 0.7) +
...
dat |> 
  ...
  scale_y_continuous("Recommendation", limit=c(1,4), n.breaks=4, labels=levels(dat$Rating)) +
...
dat2 <- 
  ...
  mutate(worthy = if_else(is.na(worthy), rating, worthy),
         new = if_else(is.na(new), rating, new), 
         supported = if_else(is.na(supported), rating, supported)) |> 
  pivot_longer(c("worthy", "new", "supported"), names_to="Scale", values_to="sub_rating") |> 
  mutate(Scale = fct_relevel(Scale, "worthy", "new", "supported"))
...
```{r}
lmm1 <- lmer(sub_rating ~ 1 + Cond*Scale + (1 | Rvwr), data=dat2, REML=FALSE,
...
```{r}
lmm2 <- lmer(sub_rating ~ 1 + Cond + Scale + (1 | Rvwr), data=dat2, REML=FALSE,
             control=lmerControl(calc.derivs=FALSE))
...
```{r}
lmm3 <- lmer(sub_rating ~ 1 + Cond + Scale + (1 + Scale | Rvwr), data=dat2, REML=FALSE,
...
sc2_1 <- mm[,6]
sc3_2 <- mm[,7]
lmm4 <- lmer(sub_rating ~ 1 + Cond + Scale + (1 + sc2_1+sc3_2 || Rvwr), data=dat2, REML=FALSE,
             control=lmerControl(calc.derivs=FALSE, check.nobs.vs.nRE="ignore"))
