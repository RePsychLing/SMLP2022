...
Due to missing scores for some tests we have somewhat less than 10,000 observtions.
...
Various options for contrast coding are the topic of the *MixedModelsTutorial\_contrasts\_emotikon.qmd* and *MixedModelsTutorial\_contrasts\_kwdyz.qmd* notebooks.
...
display(issingular(m_cpxCohort))
...
display(issingular(m_zcpCohort))
...
```{julia}
display(MixedModels.likelihoodratiotest(m_oviCohort, m_zcpCohort))
...
:::{.callout-warning}
The following LMMs `m1`, `m2`, etc. take a bit longer (e.g., close to 3 minutes in the REPL on a MacBook Pro).
:::
...
In prinicple, the models should yield the same deviance.
...
+ julia> loglikelihood(m1) # StatsBase.loglikelihood: return loglikelihood of the model
+ julia> deviance(m1)      # StatsBase.deviance: negative twice the log-likelihood relative to saturated model
+ julia> objective(m1)     # MixedModels.objective: saturated model not clear: negative twice the log-likelihood
...
```jl
pkg > add MixedModelsMakie#pa/ellipse
```

```{julia}
using Revise
shrinkageplot!(Figure(; resolution=(1000, 1200)), m1; ellipse=true)
```
