...
```{julia}
contrasts = (
      Dict(:Sex   => EffectsCoding(base= "male"; levels=["M", "F"])),
      Dict(:Cat   => EffectsCoding(base= "A"; levels=["A", "B"])),
      Dict(:Subj  => Grouping()),
      Dict(:Item  => Grouping())
   );
```
...
```{julia}
fit1a = let
     form = @formula(resp ~ 1 + Sex*Cat + (1 | Subj) + (1 | Item))
...
```{julia}
fit1b = let
     form = @formula(resp ~ 1 + sex1*cat1 + (1 | Subj) + (1 | Item))
     fit(MixedModel, form, dat1, Bernoulli(); contrasts)
end
```
...
```{julia}
display(lrtest(f1a, m_pc1, m_pc2, m_pc3))
