...
using JellyMe4         # move mixed-model object to R
using MixedModels
using MixedModelsMakie # diagnostic plots
using Random           # random number generators
using RCall            # call R from Julia
using StatsBase        # for mean()
using StatsModels      # for lrtest()
...
For a simple and complete set of observation-level and subject-level residual diagnostics we reproduce the model with `lmer` and use  `easystasts::check_model()`
...
lmer_pc0 <- lmer(llt ~ 1 + Fam + (1 | Subj), 
            data=dat, REML=FALSE, control=lmerControl(calc.derivs = FALSE))
"""
```

```{julia}
#| label: residdiagnostics
#| warning: false

R"check_model(lmer_pc0)"
```

Alternatively, we can use `JellyMe4` to transfer the fitted model object to R.

```{julia}
#| label: JellyMe4-transfer
#| warning: false

mm_pc0 = (m_pc0, dat);
@rput mm_pc0;
R"summary(mm_pc0)"
R"check_model(mm_pc0)"
```
